...
type ColumnType = ColumnDef<IntegrationInterface, unknown>;
...
export function IntegrationListPage(props: IntegrationListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getIntegrations({
        ...
        order: params.order,
        searchTermKeys: ['type.contains', 'settings.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<IntegrationInterface>>(
    () => `/integrations?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: IntegrationInterface) => {
    if (hasAccess('integration', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/integrations/view/${row.id}`);
    }
  };
